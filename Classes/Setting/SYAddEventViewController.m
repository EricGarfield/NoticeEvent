...
-(UILabel *)colorLable{
    if(_colorLable == nil){
        ...
        lable.text = @"Input event represents color";
        lable.textColor = RGB(0, 150, 30);
        lable.textAlignment = NSTextAlignmentCenter;
        _colorLable = lable;
    ...
    if(self.nameField.text.length > 0){
        ...
        if(self.selectedColor.length > 0){
            ...
            if([SYEventTool isExist:event]){
                [[[UIAlertView alloc]initWithTitle:@"Add failure" message:@"Event already extist" delegate:self cancelButtonTitle:@"Confirm" otherButtonTitles:nil, nil]show];
            }else{
                
                //4、都没问题了就可以添加进数据库
                [SYEventTool addEvent:event];
                [[[UIAlertView alloc]initWithTitle:@"Add success" message:@"Event have been added" delegate:self cancelButtonTitle:nil otherButtonTitles:@"Confirm", nil]show];
            }
        }else{
            [[[UIAlertView alloc]initWithTitle:@"Event represents color cant be none" message:@"Choose represent color" delegate:self cancelButtonTitle:@"Confirm" otherButtonTitles:nil, nil]show];
        }
    }else{
        [[[UIAlertView alloc]initWithTitle:@"Event can't be none" message:@"Input event name" delegate:self cancelButtonTitle:@"Confirm" otherButtonTitles:nil, nil]show];
    }
}
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if([alertView.title isEqualToString: @"Add success"]){
        [self back];
    }else if([alertView.title isEqualToString: @"Event represents color cant be none"]){
