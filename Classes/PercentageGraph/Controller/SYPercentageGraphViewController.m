...
-(UIButton *)startBtn{
    if(_startBtn == nil){
        ...
        [btn setTitle:@"Select start date" forState:UIControlStateNormal];
 ...
 */
-(void)refreshGraph{
    if ([self.startBtn.titleLabel.text isEqualToString:@"Select start date"] || [self.endBtn.titleLabel.text isEqualToString:@"Select end date"]) {
        return;
    }else if((self.startDate == self.endDate) || (self.startDate > self.endDate)){
        [[[UIAlertView alloc]initWithTitle:@"Search failure" message:@"Data input wrong" delegate:self cancelButtonTitle:@"Confirm" otherButtonTitles:nil, nil]show];
    }else{
