...
+ (void)initialize {
    ...
    FMResultSet *set = [_db executeQuery:@"SELECT * from t_event WHERE eventName = 'Rest' OR eventName = 'Sleeping';"];
    if(!set.next){
        [_db executeUpdate:@"INSERT INTO t_event(eventId, eventName, eventColor) VALUES (1, 'Rest', 'event_white');"];
        [_db executeUpdate:@"INSERT INTO t_event(eventId, eventName, eventColor) VALUES (2, 'Sleeping', 'event_black');"];
    }
}
...
+ (NSArray *)events {
    ...
    while (set.next) {
        ...
        if(![event.eventName isEqualToString:@"Rest"]){
